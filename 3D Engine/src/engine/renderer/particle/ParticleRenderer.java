...
public class ParticleRenderer {
	...
	protected ParticleRenderer(Matrix4f projectionMatrix){
		vbo = MainGameLoop.theLoader.createEmptyVBO(INSTANCE_DATA_LENGTH * MAX_INSTANCES);
		quad = MainGameLoop.theLoader.loadToVAO(VERTICES, 2);
		MainGameLoop.theLoader.addInstancedAttribute(quad.getVaoID(), vbo, 1, 4, INSTANCE_DATA_LENGTH, 0);
		MainGameLoop.theLoader.addInstancedAttribute(quad.getVaoID(), vbo, 2, 4, INSTANCE_DATA_LENGTH, 4);
		MainGameLoop.theLoader.addInstancedAttribute(quad.getVaoID(), vbo, 3, 4, INSTANCE_DATA_LENGTH, 8);
		MainGameLoop.theLoader.addInstancedAttribute(quad.getVaoID(), vbo, 4, 4, INSTANCE_DATA_LENGTH, 12);
		MainGameLoop.theLoader.addInstancedAttribute(quad.getVaoID(), vbo, 5, 4, INSTANCE_DATA_LENGTH, 16);
		MainGameLoop.theLoader.addInstancedAttribute(quad.getVaoID(), vbo, 6, 1, INSTANCE_DATA_LENGTH, 20);
	...
	protected void render(Map<ParticleTexture, List<Particle>> particles, Camera camera){
		...
		for(ParticleTexture texture : particles.keySet()){
			...
			MainGameLoop.theLoader.updateVBO(vbo, vboData, buffer);
	...
	private void bindTexture(ParticleTexture texture){
		...
		GL11.glBlendFunc(GL11.GL_SRC_ALPHA, texture.getBlendType());
	...
	private void updateModelViewMatrix(Vector3f position, float rotation, float scale, Matrix4f viewMatrix, float[] vboData){
		...
		Matrix4f.rotate((float)Math.toRadians(rotation), GlobalAxis.Z.toVector(),modelMatrix, modelMatrix);
